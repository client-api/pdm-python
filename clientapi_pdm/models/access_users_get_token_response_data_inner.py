...
class AccessUsersGetTokenResponseDataInner(BaseModel):
    """
    ...
    token_name: Annotated[str, Field(strict=True)] = Field(description="The token ID part of an API token authentication id.  This alone does NOT uniquely identify the API token - use a full `Authid` for such use cases.", alias="token-name")

    tokenid: Annotated[str, Field(min_length=3, strict=True, max_length=64)] = Field(description="API Token ID")

    __properties: ClassVar[List[str]] = ["comment", "enable", "expire", "token-name", "tokenid"]
    ...
    def token_name_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not isinstance(value, str):
            value = str(value)
    ...
    def tokenid_validate_regular_expression(cls, value):
        ...
        if not re.match(r"^(?:[^\s:/\x00-\x1f\x7f]+)@(?:[A-Za-z0-9_][A-Za-z0-9._\-]*)!(?:[A-Za-z0-9_][A-Za-z0-9._\-]*)$", value):
            raise ValueError(r"must validate the regular expression /^(?:[^\s:/\x00-\x1f\x7f]+)@(?:[A-Za-z0-9_][A-Za-z0-9._\-]*)!(?:[A-Za-z0-9_][A-Za-z0-9._\-]*)$/")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of AccessUsersGetTokenResponseDataInner from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of AccessUsersGetTokenResponseDataInner from a dict"""
        if obj is None:
            return None
